...
router.post('/invite', async (req, res) => {
    try {
        ...
        const adminRef = firebase_1.adminDb.collection('admins');
...
router.post('/signup', async (req, res) => {
    try {
        ...
        if (!firebase_1.adminDb || !firebase_1.adminAuth) {
        ...
        // 4. Set Custom Claim
        await firebase_1.adminAuth.setCustomUserClaims(userRecord.uid, { role: 'admin' });
        // 5. Update Firestore
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const doc = await firebase_1.adminDb.collection('admins').doc(adminId).get();
        if (!doc.exists) {
        ...
        if (uid && firebase_1.adminAuth) {
            try {
                await firebase_1.adminAuth.deleteUser(uid);
            }
            catch (authError) {
        ...
        await firebase_1.adminDb.collection('admins').doc(adminId).delete();
